...
public class DaoDeudaAutomotor extends DaoDeuda<DeudaAdapter> {

	private static final String SP_RECUPERO_DEUDA = "{call sp_veh_carga_mov_cogmvl(?)}";
	...
	private static final String SQL_RECUPERO_VENCIDA = "Select *, cc_n_transac as transaccion, (NVL(i_capital, 0.0) + NVL(i_recargo, 0.0) + NVL(i_multa, 0.0)) as i_total \n"
												     + "From tmp_veh_cons_deudv \n"
												     + "Where d_estado not like '%PLAN%' ";

	private static final String SQL_RECUPERO_TOTAL_A_VENCER = "Select *, (NVL(i_capital, 0.0) + NVL(i_recargo, 0.0) + NVL(i_multa, 0.0)) as i_total  From tmp_veh_totales";

	private static final String SQL_RECUPERO_TOTAL_VENCIDA = "Select *, (NVL(i_capital, 0.0) + NVL(i_recargo, 0.0) + NVL(i_multa, 0.0)) as i_total  From tmp_veh_totalesv";

	private static final String SQL_RECUPERO_TOTAL_GENERAL = "Select *, (NVL(i_capital, 0.0) + NVL(i_recargo, 0.0) + NVL(i_multa, 0.0)) as i_total  From tmp_veh_total_gral";

	private static final String SP_RECUPERO_DEUDA_LEGALES = "{call sp_veh_det_reaj_cogmvl( ? )}";
	
	private static final String SQL_RECUPERO_DEUDA_LEGALES = "select * from tmp_resultado";

	private static final String SP_RECUPERAR_TOTAL_CANCELACION = "{call sp_veh_deuda_pago_cogmvl(?, ? , ? , ?)}";
	
	public DaoDeudaAutomotor(Connection connection) {
		super(connection);
	}
